...
class segments {
 public:
  ...
  // capacity
  size_type size() const noexcept { return std::distance(first(), end()); }
...
auto virtual_offset_to_file_offset(SegmentIt first, SegmentIt last, AddressType address) -> AddressType {
  using SectionTraits          = traits::segment_traits<typename std::iterator_traits<SegmentIt>::value_type>;

  using difference_type = typename SectionTraits::difference_type;
  using address_type    = typename SectionTraits::address_type;

  static_assert(std::is_convertible_v<AddressType, address_type>, "address type error");

  address_type result = 0;
  for (auto iter = first; iter != last; ++iter) {
    address_type base = SectionTraits::virtual_offset(*iter);
    if (std::clamp(address, base, base + SectionTraits::virtual_size(*iter)) == address) {
      difference_type offset = address - base;
      result = SectionTraits::file_offset(*iter) + offset;
    }
  }
  return result;
}
...
auto file_offset_to_virtual_offset(SegmentIt first, SegmentIt last, AddressType address) -> AddressType {
  ...
  address_type result = 0;
  for (auto iter = first; iter != last; ++iter) {
    address_type base = SectionTraits::file_offset(*iter);
    if (std::clamp(address, base, base + SectionTraits::file_size(*iter)) == address) {
      difference_type offset = address - base;
      result = SectionTraits::virtual_offset(*iter) + offset;
    }
  }
  return result;
}
